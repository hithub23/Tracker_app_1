  ...
  <div class="max-w-screen-sm mx-auto px-4 py-10">
    ...
    <div v-if="dataLoaded">
      ...
      >
        ...
        <span
          class="mt-6 py-1.5 px-5 text-xs text-white 
          bg-at-light-green rounded-lg shadow-md"
        >
          {{ data.workoutType}}
        </span>
      ...
      <!-- Exercises -->
      <div 
        class="mt-10 p-8 rounded-md flex flex-col
...
export default {
  ...
  setup() {
    ...
    // Get workout data
      const getData = async() => {
      try{
        const {data: workouts, error} = await supabase.from("workout").select('*').eq('id', currentId)
    ...
    const deleteExercise = (id) => {
      if (data.value.exercises.length > 1) {
        data.value.exercises = data.value.exercises
