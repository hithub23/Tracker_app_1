  ...
  <!-- No Data -->
    ...
    >
      ...
      >
        ...
        <p
          class="mt-6 py-1 px-3 text-xs text-white bg-at-light-green shadow-md rounded-llg"
        >
        {{ workout.workoutType }}
        </p>

        <h1
          class="mt-8 mb-2 text-center text-xl text-at-light-green"
        >
        {{ workout.workoutName }}
        </h1>
      </router-link>
...
export default {
  ...
  setup() {
    ...
    const getData = async() => {
      try{
        const {data: workouts, error} = await supabase.from("workout").select('*')
